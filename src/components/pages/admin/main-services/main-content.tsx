...
function MainContentMainServices() {
  ...
  // [DELETE] MainServices
  const deleteMainServices = ApiDeleteMainServices({
    onSuccess() {
      setDeletePopup({
        open: false,
        selectedId: null,
      });
      alert("Berhasil menghapus Data MainServices");
    },
    onError(err) {
      setDeletePopup({
        open: false,
    ...
    () => [
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex flex-row items-center space-x-4 text-right">
              <Link href={`/admin/main-services/services-all/${id}`}>
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex flex-row items-center space-x-4 text-right">
              <Link href={`/admin/main-services/edit/${id}`}>
                <ButtonIcon>
                  <EditIcon color="blue" />
                </ButtonIcon>
              </Link>
    ...
    <div className="flex h-full flex-col gap-y-8 pt-4 md:ml-4 md:mr-0">
      <div className="grid gap-4 md:flex">
        ...
        <div className="flex justify-end">
          <Link href="/admin/main-services/add">
            <Button
              type="button"
      ...
      <Confirm
        open={deletePopup.open}
        onOk={() => deleteMainServices.mutate(deletePopup.selectedId!)}
...
export default MainContentMainServices;
